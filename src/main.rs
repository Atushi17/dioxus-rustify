    ...
    if lhs.len() == rhs.len() {
        for (lp, rp) in lhs.iter().zip(rhs.iter()) {
            if lp.starts_with(':') && lp.len() > 1 {
                ...
                params.insert(
                    key.to_string(),
                    serde_json::Value::String(decode_url_param(rp)),
                );
...
fn package_url(cfg: &RuntimeConfig) -> String {
    format!(
        "{}/api/compiler/package/{}",
        cfg.api_base_url, cfg.compile_id
    )
}
            ...
            if !route_entries().is_empty() {
                ...
                for candidate in candidates.iter() {
                    if let Some(hit) = route_entries().iter().find(|r| &r.path == candidate) {
                        matched_route =
                            Some((hit.page_id.clone(), hit.path.clone(), candidate.clone()));
                        break;
                    }
                }
                if matched_route.is_none() {
                    for candidate in candidates.iter() {
                        if let Some(hit) = route_entries()
                            .iter()
                            .find(|r| route_matches(&r.path, candidate))
                        {
                            matched_route =
                                Some((hit.page_id.clone(), hit.path.clone(), candidate.clone()));
                            break;
                ...
                if matched_route.is_none() {
                    if let Some(def_route) = default_route() {
                        if let Some(hit) = route_entries().iter().find(|r| r.path == def_route) {
                            matched_route =
                                Some((hit.page_id.clone(), hit.path.clone(), def_route.clone()));
                        }
                    }
                }
...

    {
        ...
        use_effect(move || {
            ...
            if let Some(page_id) = current_page_id() {
                if !page_cache.peek().contains_key(&page_id)
                    && !in_flight_fetches.peek().contains(&page_id)
                {
                    ...
                    spawn(async move {
                        ...
                        match fetch_json(&url).await {
                            Ok(value) => match serde_json::from_value::<ComponentNode>(value) {
                                ...
                                Err(e) => {
                                    page_error
                                        .set(Some(format!("failed to parse page {}: {}", id, e)));
                                }
                            },
                            Err(e) => {
                                page_error.set(Some(format!("failed to fetch page {}: {}", id, e)));
                            }
                        }
        ...
        Some(Ok(_)) => {
            ...
            if let Some(page_id) = page_id_opt {
                if let Some(page) = page_cache().get(&page_id).cloned() {
                    ...
                    let hide_shell = shell
                        .hide_shell_on_pages
                        .iter()
                        .any(|hidden| hidden == &page_id)
                        || current_route
                            .as_ref()
                            .map(|route| {
                                shell
                                    .hide_shell_on_routes
                                    .iter()
                                    .any(|hidden| normalize_route(hidden) == normalize_route(route))
                            })
                            .unwrap_or(false);

                    let (left_sidebar, right_sidebar): (Vec<ComponentNode>, Vec<ComponentNode>) =
                        shell
                            .sidebar_children
                            .clone()
                            .into_iter()
...
fn test_parse_repeater() {
    ...
    fn find_repeater(node: &ComponentNode) {
            ...
            .unwrap_or(false)
        {
            ...
            println!(
                "REPEATER DS: {:?}",
                node.props.extra.get("repeaterDataSource")
            );
        }
...
fn test_resolve_array_path() {
    ...
    global_data
        .as_object_mut()
        .unwrap()
        .insert("re_result".to_string(), response_json);
